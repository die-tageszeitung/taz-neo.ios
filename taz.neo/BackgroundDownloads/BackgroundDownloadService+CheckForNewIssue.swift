...
#warning("TODO: NOT CALLED YET")

/// MARK: - Application Restart Handling
extension BackgroundDownloadService {
  
  /// Restart pending downloads for unfinished issues in App Foreground State
  /// Pending Downloads can occour if App was terminated or download not started yet
  /// Example: no download worked....>100 pending downloads > app un-useable
#warning("TODO: NOT CALLED YET")
  func restartPendingDownloadsOnMain1() {
    ///ensure main Thread otherwise a crass may occour
    guard Thread.isMainThread else {
      log("Not on main thread, NOT restarting pending downloads...")
      return
    }
    log("restartPendingDownloads...")
    guard let storedFeed = feederContext?.defaultFeed else {
      log("No stored feed available, cannot restart downloads.")
      return
    }
    
    let pendingIssues = StoredIssue.unfinishedAutodownloading(feed: storedFeed)
    guard !pendingIssues.isEmpty else {
      log("No pending issues to restart.")
      return
    }
    ...
    if let netAvailability = feederContext?.netAvailability,
       netAvailability.isConnected {
      connected = true
    ...
    log(">>> pendingIssues: \(pendingIssues.count) / connected: \(connected), WiFi: \(isOnWiFi), Cellular: \(isOnCellular)")
    
    for issue in pendingIssues {
      //      if issue.date.issueKey == currentIssue?.date.issueKey {
      //        log("Already downloaded issue \(issue.date.short), skipping...")
      //        continue
      //      }
      
      if !isOnWiFi && autoloadOnlyInWLAN {
        // Download only the moment using IssueOverviewService
        TazAppEnvironment.sharedInstance.service?.apiLoadMomentImages(for: issue, isPdf: isFacsimile)
        continue
      }
      
      // Download the full issue
      guard let zipUrl = issue.zipUrl else {
        log("No zip URL for issue \(issue.date.short), skipping...")
        continue
      }
      
      do {
        let session = try BackgroundSession(zipUrl, asBackgroundSession: false) { [weak self] url, error in
          if url != zipUrl {
            self?.log("ERRROR: URL mismatch for audio download \(url) != \(zipUrl)")
          }
          self?.dlCallback(downloadUrl: zipUrl, err: error)
        }
        session.allowMobile = !autoloadOnlyInWLAN
        log("Restarting download for issue \(issue.date.short) from URL: \(zipUrl)")
        session.downloadZip(toDir: issue.tempDir.path)
      } catch {
        log("Failed to restart download for issue \(issue.date.short): \(error)")
      }
      
      // Add audio download logic
      if autoloadAudio, let audioUrl = issue.zipAudioUrl {
        do {
          let audioDownloadSession = try BackgroundSession(audioUrl, asBackgroundSession: false) { [weak self] url, err in
            if url != audioUrl {
              self?.log("ERRROR: URL mismatch for audio download \(url) != \(audioUrl)")
            }
            self?.log("Audio Download finished with Error: \(err ?? "-")")
          }
          audioDownloadSession.allowMobile = !autoloadOnlyInWLAN
          log("Restarting audio download for issue \(issue.date.short) from URL: \(audioUrl)")
          audioDownloadSession.downloadZip(toDir: issue.dir.path)
        } catch {
          log("Failed to restart audio download for issue \(issue.date.short): \(error)")
        }
      }
    }
  }
